...
agg_climate_data <- function(year, data_raster, climate_var, daily_agg, trans = 'polynomial', trans_specs, geoweights_table, second_weights = FALSE) {
  ...
  # Binned
  if(trans == 'binned'){
    num_bins <- trans_specs[1]
    bin_width <- trans_specs[2]
    center <- trans_specs[3]
  }
...
#Create function to come before transformation
aggregate_setup <- function(data_raster, climate_var, daily_agg, geoweights_table, second_weights = FALSE){
  # Function to convert raster to data.table from https://gist.github.com/etiennebr/9515738
  as.data.table.raster <- function(x, row.names = NULL, optional = FALSE, xy=FALSE, inmem = raster::canProcessInMemory(x, 2), ...) {
    if(inmem) {
      v <- data.table::as.data.table(raster::as.data.frame(x, row.names=row.names, optional=optional, xy=xy, ...))
...
# Function to come after transformation
staggregate_finish <- function(clim_dt, weights_dt, list_names){
...
staggregate_poly <- function(data_raster, climate_var, daily_agg, k, geoweights_table, second_weights, year){
  ...
  #Get necessary objects from the first function
  setup_list <- aggregate_setup(data_raster, climate_var, daily_agg, geoweights_table, second_weights)
  ...
  r <- lapply(poly_orders, FUN=function(x){clim_daily ^ x})
    ...
    sum_by_poly <- staggregate_finish(clim_dt, weights_dt, list_names, year)
